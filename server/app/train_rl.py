...
# if __name__ == '__main__':  # 221021 fk: added these two lines in reaction
#     freeze_support()        # to an EOFError raised when executing the script

def train_rl(environment, model, q0, q1, q2):
    # TODO: send buffer data into queue q1
    # send initial bufferdata into q1:

    if model:
        environment = model.split('_')[0]

    # Generate environments
    envs = []
    use_pixel = 'pixel' in args.arch
    for i in range(args.procs):
        env = gym.make(environment)  # fk: replaced args.env with environment
        if use_pixel:
            env = RGBImgPartialObsWrapper(env)
        a = env.seed(100 * args.seed + i)
        envs.append(env)
    ...
    model_name_parts = {
        'env': environment,  # fk: replaced args.env with environment
        'algo': args.algo,
        'arch': args.arch,
        'instr': instr,
        'mem': mem,
    ...
    args.model = model.format(**model_name_parts) if model else default_model_name  # fk: replace args.model with model
    ...
    best_success_rate = 0
    best_mean_return = 0
    test_env_name = environment  # fk: replaced args.env with environment

    while status['num_frames'] < args.frames:
        # Update parameters

        update_start_time = time.time()
        # logs, vis_data, amount_feedback_frames, episodes_done = algo.update_parameters(q2)
        logs, vis_data, amount_feedback_frames, episodes_done, buffer_counter = algo.run_study(q2)
        ...
        # fk: sending dataframe with random episodes into queue:
        if vis_data is not None:
            print('Putting data into queue')
            q1.put(
                (vis_data.to_dict(), status['num_frames'] / args.frames * 100, amount_feedback_frames, episodes_done, buffer_counter))

        # Save obss preprocessor vocabulary and model

        if args.save_interval > 0 and status['i'] % args.save_interval == 0 or q0.qsize() > 0:
            # obss_preprocessor.vocab.save()
        ...
        # if status['num_frames'] in [20 * 2560, 40 * 2560, 60 * 2560, 80 * 2560, 100 * 2560, 120 * 2560, 140 * 2560,
        #                             160 * 2560, 180 * 2560, 200 * 2560, 220 * 2560, 240 * 2560, 260 * 2560,
        #                             280 * 2560, 300 * 2560, 320 * 2560, 340 * 2560, 360 * 2560, 380 * 2560, 400 * 2560]:
        #     obss_preprocessor.vocab.save()
        #     with open(status_path, 'w') as dst:
        #         json.dump(status, dst)
        #         utils.save_model(acmodel, args.model)
        ...
        #         save_model = True
        #     if save_model:
        #         utils.save_model(acmodel, args.model + '_best' + str(status['num_frames']*2))
        #         obss_preprocessor.vocab.save(utils.get_vocab_path(args.model + '_best' + str(status['num_frames']*2)))
        #         logger.info("Return {: .2f}; best model is saved".format(mean_return))
        #     else:
        #         logger.info("Return {: .2f}; not the best model; not saved".format(mean_return))
        #
    ...
    # after the training has stopped, we wait for the user to click the stop button:
    while q0.qsize() == 0:
        time.sleep(1)

    # # now we save the model and the vocab:
    # obss_preprocessor.vocab.save()
    # with open(status_path, 'w') as dst:
    #     json.dump(status, dst)
    #     utils.save_model(acmodel, args.model)
    ...
    # file = f'buffer_{default_model_name}.pkl'
    # with open(file, 'wb') as f:
    #     pickle.dump(algo.proc_dfs, f)
