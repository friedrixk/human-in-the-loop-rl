...
def get_data(path, regex):
    ...
    for model in models:
        ...
        #success_rate = success_rate.rolling(args.window, center=True).mean() #original
        success_rate = success_rate.rolling(args.window, center=True, min_periods=1).mean() #changed
