...
from app.generate_action_sequence import ActionSequenceGenerator
...
parser.add_argument("--HITL", default=False,
                     help= "demo generation mode: human or BotAgent")
parser.add_argument(
    '--agent_view',
    default=False,
...
def evaluate_agent(il_learn, eval_seed, num_eval_demos, return_obss_actions=True):  #ps: changed; original: return_obss_actions=False
    """
    Evaluate the agent on some number of episodes and return the seeds for the
    episodes the agent performed the worst on.
    """
     #field names
    fields = ["fail_seed", "fail_actions", "return", "positions"] 
    fields_returns = ["return"]
    ...
    filename = "evaluate_agent_" + date_time + ".csv" 
    filename_returns = "evaluate_agent_returns" + date_time + ".csv"   

    logger.info("Evaluating agent on {} using {} demos".format(il_learn.args.env, num_eval_demos))
    print("Evaluating agent on {} using {} demos".format(il_learn.args.env, num_eval_demos))
    ...
    rows_returns.append(logs["return_per_episode"]) 
    ...
    for idx, ret in enumerate(logs["return_per_episode"]):
        if ret <= 0:
            ...
            new_row = []
            new_row.append(logs["seed_per_episode"][idx])
            #new_row.append(logs["observations_per_episode"][idx])
            new_row.append(logs["actions_per_episode"][idx])
            new_row.append(logs["return_per_episode"][idx])
            #new_row.append(logs["positions_per_episode"][idx])
            rows.append(new_row)
    ...
    # writing second csv file containing all returns
    with open(filename_returns, 'w') as csvfile:
      # creating a csv writer object
       csvwriter = csv.writer(csvfile)
      
       # writing the fields
       csvwriter.writerow(fields_returns)
      
       # writing the data rows
       csvwriter.writerows(rows_returns) 
    ...
    if not return_obss_actions:
        ...
        print("returns per episode: ", logs['return_per_episode']) 
        print("fail seeds: {}".format( fail_seeds))     
        print("fail actions: {}".format( fail_actions)) 
        print("number of fail action sequences: ", len(fail_actions)) 
        plot_grid.set_plotdata(args.env, fail_seeds, logs['return_per_episode'], fail_actions)
        plot_grid.create_plotgrid()
...
def generate_demos(env_name, seeds):
    ...
    for seed in seeds:
       # Run the expert for one episode
        ...
        # human generates new demos
        if args.HITL:
            action_seq = []
            action_seq_generator = ActionSequenceGenerator(env_name, seed, args.agent_view, action_seq)
            action_seq = action_seq_generator.start_generation(env_name)

            for i in range(len(action_seq)):
               new_obs, reward, done, _ = env.step(action_seq[i])
            #agent.analyze_feedback(reward, done)

               actions.append(action_seq[i])
...
def grow_training_set(il_learn, train_demos, eval_seed, grow_factor, num_eval_demos):
    ...
    while len(train_demos) < new_train_set_size:
        ...
        success_rate, fail_seeds, _, _ = evaluate_agent(il_learn, eval_seed, num_eval_demos)
