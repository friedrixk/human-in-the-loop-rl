...
class RoomGridLevel(RoomGrid):
    ...
    def reset(self, **kwargs):
        ...
        multiplier = args.max_steps_mult
        self.max_steps = self.max_steps * multiplier
...
class LevelGen(RoomGridLevel):
    ...
    def __init__(
        ...
        action_kinds=['goto', 'pickup', 'open', 'putnext', 'flip', 'uncover'],
    ...
    def rand_instr(
        ...
        if kind == 'action':
            ...
            if action == 'goto':
                ...
                return FlipInstr(self.rand_obj(types=['switch']))
            elif action == 'uncover':
                return UncoverInstr(self.rand_obj(types=['unspecified_door']))
