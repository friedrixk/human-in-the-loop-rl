...
OBJ_TYPES = ['box', 'ball', 'key', 'door', 'switch', 'unspecified_door']
...
class ObjDesc:
    ...
    def surface(self, env):
        ...
        #added exception so that uncover instructions don't contain the color to be uncovered
        if self.color and self.type != 'unspecified_door':
            s = self.color + ' ' + s
...
class FlipInstr(OpenInstr):
    def __init__(self, obj_desc, strict=False):
        #super().__init__(obj_desc, strict=False)
        assert obj_desc.type == 'switch'
        self.desc = obj_desc
        self.strict = strict

    def surface(self, env):
        return 'flip ' + self.desc.surface(env)


class UncoverInstr(ActionInstr):
    def __init__(self, obj_desc, strict=False):
        super().__init__()
        assert obj_desc.type == 'unspecified_door'
        self.desc = obj_desc
        self.strict = strict

    def surface(self, env):
        return 'uncover ' + self.desc.surface(env)
    ...
    def verify_action(self, action):
        ...
        for e in self.desc.obj_set:
            if front_cell and front_cell is e and not e.is_unspecified:
                return 'success'
